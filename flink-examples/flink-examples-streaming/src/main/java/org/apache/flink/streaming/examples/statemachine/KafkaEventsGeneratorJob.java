...
public class KafkaEventsGeneratorJob {
	...
	public static void main(String[] args) throws Exception {
		...
		int sleep = params.getInt("sleep", 5000);
		...
		producerProp.setProperty(ProducerConfig.RETRIES_CONFIG, "3");
		producerProp.setProperty(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, brokers);
		final StreamExecutionEnvironment env = StreamExecutionEnvironment.getExecutionEnvironment();
		env.setParallelism(2);
		env
			.addSource(new EventsGeneratorSource(errorRate, sleep))
			.addSink(new FlinkKafkaProducer010<>(kafkaTopic, new EventDeSerializer(), producerProp, new FlinkFixedPartitioner<>()));
