 ...
 * 在本地嵌入式的Flink运行实例上运行Flink程序的计划执行器
...
public class LocalExecutor extends PlanExecutor {
		...
		final MiniClusterConfiguration miniClusterConfiguration = new MiniClusterConfiguration.Builder()
			...
			.setNumTaskManagers(
				configuration.getInteger(
					...
					ConfigConstants.DEFAULT_LOCAL_NUMBER_TASK_MANAGER)) // 配置taskManager数量
			.setRpcServiceSharing(RpcServiceSharing.SHARED) // 使用共享rpc服务
			.setNumSlotsPerTaskManager(
				configuration.getInteger(
					TaskManagerOptions.NUM_TASK_SLOTS, 1)) // 设置taskManager中的slot数量
			.build();
	...
	private Configuration createConfiguration() {
		...
		newConfiguration.setInteger(TaskManagerOptions.NUM_TASK_SLOTS, getTaskManagerNumSlots()); // 根据plan中的最大并行度设置 taskManager中的slot数量
		newConfiguration.setBoolean(CoreOptions.FILESYTEM_DEFAULT_OVERRIDE, isDefaultOverwriteFiles()); // 当文件系统中的文件存在时是否覆盖,默认false
